...
func (a *RemoteOciPuller) PullComponentSHA(ctx context.Context, componentName, tag, destPath string, platform simpleplatform.Platform) (*ocispec.Descriptor, error) {
	return a.pullOCI(ctx, ociconsts.ComponentRepoPrefix+componentName, tag, destPath, platform)
}
...
func (a *RemoteOciPuller) PullComponentSHAByFullPath(ctx context.Context, componentPath, tag, destPath string, platform simpleplatform.Platform) (*ocispec.Descriptor, error) {
	return a.pullOCI(ctx, componentPath, tag, destPath, platform)
}
...
func (a *RemoteOciPuller) pullOCI(ctx context.Context, repo, tag, destPath string, platform simpleplatform.Platform) (*ocispec.Descriptor, error) {
	src, err := a.cachedRepo(fmt.Sprintf("%s/%s", a.remote.Registry, repo))
	if err != nil {
		return nil, err
	}
