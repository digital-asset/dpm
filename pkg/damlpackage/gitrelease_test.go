...
func TestExpandReleaseGitDependencies(t *testing.T) {
	const tag = "v1.0.0"
	...
	t.Setenv("DPM_TEST_GITHUB_RELEASE_HOST", "github.com")

	releaseLine := "git:github.com/org/repo.git?release=" + tag
	otherLine := "oci://example.com/foo:1.0.0"

	expanded, err := ExpandReleaseGitDependencies(context.Background(), []string{otherLine, releaseLine})
	require.NoError(t, err)
	require.Len(t, expanded, 3)
	assert.Equal(t, otherLine, expanded[0])
	assert.Equal(t, FormatGitReleaseLine("github.com/org/repo", tag, assetA), expanded[1])
	assert.Equal(t, FormatGitReleaseLine("github.com/org/repo", tag, assetB), expanded[2])
}
...
func TestExpandGitReleaseDependenciesInYaml_doesNotDuplicateExistingAssets(t *testing.T) {
	...
	srv := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		if strings.Contains(r.URL.Path, "/releases/tags/") {
			_ = json.NewEncoder(w).Encode(map[string]any{
				"assets": []map[string]string{{"name": assetA}, {"name": assetB}},
			})
			return
		}
		http.NotFound(w, r)
