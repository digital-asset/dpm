...
// IsGitDependencyInput reports whether raw looks like a git dependency input.
func IsGitDependencyInput(raw string, requireGitPrefix bool) bool {
	_, err := CoerceGitDependencyInput(raw, GitInputOptions{RequireGitPrefix: requireGitPrefix})
	return err == nil
}
...
// CanonicalizeGitDependencyLines rewrites git dependency one-liners to canonical form.
func CanonicalizeGitDependencyLines(lines []string) ([]string, bool, error) {
...
func splitHostFirstBase(basePart string) (host, repoPath string) {
	slash := strings.Index(basePart, "/")
	if slash < 0 {
		return "", ""
	}
	host = basePart[:slash]
	repoPath = strings.TrimSuffix(basePart[slash+1:], ".git")
	return host, repoPath
}
